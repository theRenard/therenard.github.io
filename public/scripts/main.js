...
function Trianglify(f) {
  ...
  var f = Object.keys(Trianglify.colorbrewer),
      e = Trianglify.colorbrewer[f[Math.floor(Math.random() * f.length)]];f = Object.keys(e);var d = e[f[Math.floor(Math.random() * f.length)]];return d;
  ...
  this.options = f, this.width = e, this.height = d, this.polys = this.generatePolygons(), this.svg = this.generateSVG();var a = new XMLSerializer();this.svgString = a.serializeToString(this.svg), this.base64 = btoa(this.svgString), this.dataUri = "data:image/svg+xml;base64," + this.base64, this.dataUrl = "url(" + this.dataUri + ")";
  ...
  return function (c, b) {
    var t = d3.scale.linear().range(f).domain(d3.range(0, d, d / f.length)),
        r = d3.scale.linear().range(e).domain(d3.range(0, a, a / e.length));return d3.interpolateRgb(t(c), r(b))(.5);
  };
  ...
  var f = this.options,
    ...
    var a = d % e,
        ...
        t = Math.round(-f.bleed + c * f.cellsize + Math.random() * (f.cellsize - 2 * f.cellpadding) + f.cellpadding);return [b, t];
...
jQuery(document).ready(function ($) {
	console.log($);
	window.$body = $('body'), window.$logo = $('#logo'), window.$container = $('#container'), window.$background = $('#background');
...
$(window).load(function () {
	...
	tween.to(window.$body, .5, { opacity: 1 }, 'tween#1').from(window.$body, .5, { scale: 0.90, ease: Back.easeOut }, 'tween#1').from(window.$logo, .5, { y: +200, opacity: 0, scale: 0.90, ease: Back.easeOut }, 'tween#2').staggerTo($div, .5, { y: -50, scale: 0.5, opacity: 1, rotation: 360, delay: 1, ease: Back.easeOut }, 0.1, 'tween#2').staggerTo($a, .5, { opacity: 1, delay: 0, ease: Back.easeOut }, 0.1, 'tween#2');
